...
export default function ProductDetail(){
  ...
  return (
    <div style={{maxWidth:900,margin:'0 auto',padding:20}}>
      <h1>{product.title}</h1>
      {product.image && <img src={product.image} alt={product.title} style={{width:300,height:220,objectFit:'cover',borderRadius:6}}/>}
      <p style={{fontWeight:700}}>{product.price}</p>
      <p>{product.description}</p>

      <AdsensePlaceholder position="product-detail" />

      <a href={link} target="_blank" rel="noopener noreferrer" style={{display:'inline-block',padding:'10px 14px',background:'#0a63a9',color:'#fff',borderRadius:6,textDecoration:'none'}}>Go to seller</a>
    </div>
  );
}
